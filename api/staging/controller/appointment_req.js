...
{
  winston.info(chalk.green("*** Staging Cron Job Running For Booking Start Reminder ***"));
...
{
  winston.info(chalk.green("*** Staging Cron Job Running For Uncaputure Payment ***"));
  pool.getConnection(function(err,connection){
  if(!err)
  {
...
{
  winston.info(chalk.green("*** Staging Cron Job Running For CompleteAppointment ***"));
  pool.getConnection(function(err,connection){
      if(!err)
      {
          connection.query('select * from appointment_request where status="accept" and uncaptured=1',
...
{
    winston.info(chalk.green("*** Staging Cron Job Running For Cancel Appointment ***"));
    pool.getConnection(function(err,connection){
        if(!err)
        {
...
{
	console.log("*** Staging Requested for Start Appoinement ***");
	logger.info('*** Staging Requested for Start Appoinement ***');
...
{
		console.log("*** Staging Requested by customer for Rate... ***");
		logger.info('*** Staging Requested by customer for Rate... ***');
		pool.getConnection(function(err,connection){
			if(!err)
			{
...
{
		console.log("*** Staging Requested by Professional for Rate... ***");
		logger.info('*** Staging Requested by Professional for Rate... ***');
		pool.getConnection(function(err,connection){
			if(!err)
			{
					email=req.user.email
...
{
		console.log('*** Staging Requesting for complite Appintment ***');
		logger.info('*** Staging Requesting for complite Appintment ***');
		pool.getConnection(function(err,connection){
		if(!err)
		{
		    receivedValues = req.body
...
exports.create=function(req,res)
{
	...
	logger.info('*** Staging Requested for Creating New Appointment request... ***');
...
{
	console.log("*** Staging Requested for Editing Appointment request... ***");
	logger.info('*** Staging Requested for Editing Appointment request... ***');
	  receivedValues = req.body    //DATA FROM WEB
    if(JSON.stringify(receivedValues) === '{}')
		{
        Errors.EmptyBody(res);
...
{
	console.log("*** Staging Staging Requested for View New Appointment request... ***");
	logger.info('*** Staging Requested for View New Appointment request... ***');
	  receivedValues = req.body    //DATA FROM WEB
    if(JSON.stringify(receivedValues) === '{}')
		{
        Errors.EmptyBody(res);
...
exports.appointments=function(req,res)
{
	...
	logger.info('*** Staging Geting User Appointments ***');
	recevedvalue=req.body
...
{
		console.log("*** Staging Requested for Cancel Appointment request by customer ... ***");
		logger.info('*** Staging Requested for Cancel Appointment request by customer ... ***');
		receivedValues = req.body    //DATA FROM WEB
    if(JSON.stringify(receivedValues) === '{}')
		{
        Errors.EmptyBody(res);
...
{
	console.log('*** Staging Geting Cancel Appointment***');
	logger.info('*** Staging Geting Cancel Appointment***');
    receivedValues = req.body    //DATA FROM WEB
              ...
              if (err)
             {
              ...
              else
              {
                    ...
                    function chargeUser(is_professional, timeDifference, chargeId, amount, userId)
                    {
                        ...
                        {
                              connection.query('select a.*,p.* from appointment_request a,users p where a.appointment_request_id=? and p.id=a.professional_id',[req.body.appointment_request_id],
                              function(err,profdata){
                                  if(!err)
                                  {
                                      ...
                                      recure_id=profdata[0].recure_id
...
{
	console.log("*** Staging Requested for Accept Appointment ... ***");
	logger.info('*** Staging Requested for Accept Appointment ... ***');
	  receivedValues = req.body;    //DATA FROM WEB
    if(JSON.stringify(receivedValues) === '{}')
		{
        Errors.EmptyBody(res);
...
{
		console.log("*** Staging Requested for Appointment Reject... ***");
		logger.info('*** Staging Requested for Appointment Reject... ***');
		receivedValues = req.body    //DATA FROM WEB
    if(JSON.stringify(receivedValues) === '{}')
		{
        Errors.EmptyBody(res);
